	...
	<body>
		...
		<div id="show-activite" class="content scaffold-show" role="main">
			...
			<ol class="property-list activite">
				...
				<g:if test="${activiteInstance?.evenements}">
				<li class="fieldcontain">
					<span id="evenements-label" class="property-label"><g:message code="activite.evenements.label" default="Evenements" /></span>
					
						<g:each in="${activiteInstance.evenements}" var="e">
						<span class="property-value" aria-labelledby="evenements-label"><g:link controller="events" action="show" id="${e.id}">${e?.encodeAsHTML()}</g:link></span>
						</g:each>
					
				</li>
				</g:if>
			
				<g:if test="${activiteInstance?.passionnees}">
				<li class="fieldcontain">
					<span id="passionnees-label" class="property-label"><g:message code="activite.passionnees.label" default="Passionnees" /></span>
					
						<g:each in="${activiteInstance.passionnees}" var="p">
						<span class="property-value" aria-labelledby="passionnees-label"><g:link controller="user" action="show" id="${p.id}">${p?.encodeAsHTML()}</g:link></span>
						</g:each>
